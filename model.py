...
class Model(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim, num_layers, d_rate_input, d_rate_hid, TEXT):
        super(Model, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.embedding.weight.data.copy_(TEXT.vocab.vectors)
        self.gru = nn.GRU(embedding_dim, hidden_dim, num_layers=num_layers, 
                        bidirectional=True, dropout=d_rate_hid)
        self.dense = nn.Linear(2 * hidden_dim, output_dim)
        self.softmax = nn.Softmax(dim=1)
        self.dropout_input = nn.Dropout(d_rate_input)
        self.dropout_hidden = nn.Dropout(d_rate_hid)
        self.attention = Self_Attention(2 * hidden_dim)
    
    def init_hidden(self):
        # ここの使い方を調べる: GRUは自動的に初期化されるが...
        pass
    
    def forward(self, x):
        # x: (sentence_length, batch_size)

        embedded = self.dropout_input(self.embedding(x))
